...
void WASAOutput::CreateNtuples() {
  ...
  analysisManager->CreateNtupleIColumn("tracker_PDG",fTrackerPDGVec);
  analysisManager->CreateNtupleDColumn("tracker_dEdxTruth", fTrackerdEdxTruthVec);
  ...
  analysisManager->CreateNtupleDColumn("tracker_X", fTrackerXVec);
  analysisManager->CreateNtupleDColumn("tracker_Y", fTrackerYVec);
  analysisManager->CreateNtupleDColumn("tracker_Z", fTrackerZVec);
  analysisManager->CreateNtupleDColumn("tracker_XDir", fTrackerXDir);
  analysisManager->CreateNtupleDColumn("tracker_YDir", fTrackerYDir);
  analysisManager->CreateNtupleDColumn("tracker_ZDir", fTrackerZDir);
  analysisManager->CreateNtupleDColumn("tracker_dEdx", fTrackerdEdx);
  analysisManager->CreateNtupleDColumn("tracker_Time",fTrackerTimeVec);
...
void WASAOutput::CreateHistograms()
{
  ...
  analysisManager->SetH1XAxisTitle( 0, "E_{smeared}/E_{true}" );
...
void WASAOutput::SaveTrack( SaveType aWhatToSave, G4int aPartID,  G4int aPDG, G4double aETruth,
                             G4ThreeVector aVector, G4ThreeVector aDirection, G4double aResolution, 
   ...
   break;
    ...
    case WASAOutput::eSaveTracker: {
     fTrackerPDGVec.push_back(aPDG);
     fTrackerdEdxTruthVec.push_back(aETruth);//for trakcks, this is the truth dE/dx, not energy
     fTrackerResVec.push_back(aResolution);
     fTrackerEffVec.push_back(aEfficiency);
     fTrackerXVec.push_back(aVector.x());
     fTrackerYVec.push_back(aVector.y());
     fTrackerZVec.push_back(aVector.z());
     fTrackerXDir.push_back(aDirection.x());
     fTrackerYDir.push_back(aDirection.y());
     fTrackerZDir.push_back(aDirection.z());
     fTrackerdEdx.push_back(aEnergy); //for trakcks, this is the dE/dx, not energy
