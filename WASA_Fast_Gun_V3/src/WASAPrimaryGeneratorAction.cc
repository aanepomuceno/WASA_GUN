...
WASAPrimaryGeneratorAction::WASAPrimaryGeneratorAction() : G4VUserPrimaryGeneratorAction(), fParticleGun(nullptr), fFileName("file.txt"), fFileType(0) {
...
void WASAPrimaryGeneratorAction::GeneratePrimaries( G4Event* anEvent ) {
    ...
    if (fFileType == 1) iss >> index >> pdg >> kineticEnergy  >> x >> y >> z >> ux >> uy >> uz >> time >>  weight >> pol_x >> pol_y >> pol_z >> std::hex >> evt1 ;
    ...
    //read next particle in the event (and keep record of the current line position in the file, for next event)
     ...
     std::getline(fInputFile, line);
     std::istringstream iss(line);
     if (fFileType == 1) iss >> index >> pdg >> kineticEnergy  >> x >> y >> z >> ux >> uy >> uz >> time >>  weight >> pol_x >> pol_y >> pol_z >> std::hex >> evt2 ;
     else iss >> index >> pdg >> kineticEnergy  >> x >> y >> z >> ux >> uy >> uz >> time >> std::hex >> evt2 ;
     
      //check if end of file was reached
       if (fInputFile.eof()) {
         std::cout << "End of input file reached at event " << anEvent->GetEventID() << std::endl;
         break;
       }
     
    }
    if ( !fInputFile.eof() ) fInputFile.seekg(oldpos); //back to the previous line file
