...
//
/// \file WASAOutput.hh
/// \brief Definition of the WASAOutput class
...
class WASAOutput {
  public:
    ...
    /// Allows the access to the unique WASAOutput object.
    /// @return A pointer to the WASAOutput class.
    static WASAOutput* Instance();
    ...
    /// energy deposits, etc.). To be called for each event in WASAEventAction.
    void CreateNtuples();
    
    /// Creates histograms to combine information from all the events in the run.
    /// To be called for each run in WASARunAction.
    ...
    ///                WASAOutput::SaveType::eEMCal or WASAOutput::SaveType::eHCal).
    void SaveTrack( SaveType aWhatToSave, G4int aPartID,  G4int aPDG, G4double aETruth,
                    G4ThreeVector aVector, G4ThreeVector aDirection, G4double aResolution = 0,
  ...
  std::vector<G4int>    fTrackerPDGVec;
  std::vector<G4double> fTrackerdEdxTruthVec;
  std::vector<G4double> fTrackerResVec;
  std::vector<G4double> fTrackerEffVec;
  std::vector<G4double> fTrackerXVec;
  std::vector<G4double> fTrackerYVec;
  std::vector<G4double> fTrackerZVec;
  std::vector<G4double> fTrackerXDir;
  std::vector<G4double> fTrackerYDir;
  std::vector<G4double> fTrackerZDir;
  std::vector<G4double> fTrackerdEdx;
  std::vector<G4double> fTrackerTimeVec;
  ...
  std::vector<G4double> fHcalTimeVec;
    ...
    /// The pointer to the only WASAOutput class object.
    static WASAOutput* fWASAOutput;
