...
//
/// \file WASASmearer.hh
/// \brief Definition of the WASASmearer class
...
class WASASmearer {
  public:
    ...
    //For attrib the Potencial Gas value on Bethe-Bloch Function - (WVS) - 2025-07-09
    enum PotentialGas { eArCO2_9010, eArCO2_8020, eAir };

    /// Allows the access to the unique WASASmearer class object.
    /// @return A pointer to the WASASmearer class.
    static WASASmearer* Instance();
...
//    G4double SmearEnergy( const G4Track* aTrack, G4double aResolution = -1 );
    ...
    G4double BetheBloch( G4ParticleDefinition* aParticle, G4double aKinectE, PotentialGas aPotentialGas);
  ...
  private:
    ...
    /// A pointer to WASASmearer object.
    static WASASmearer* fWASASmearer;
